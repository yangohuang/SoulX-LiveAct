...
def allocate_kv_caches(token_count, step_count, layer_count, device, fp8,
    ...
    def make_caches():
        return {
            step: {
                layer: {
                    "k": torch.zeros(kv_shape, dtype=dtype, device=device),
                    "v": torch.zeros(kv_shape, dtype=dtype, device=device),
                    "k_scale": torch.ones(scale_shape, dtype=torch.float32, device=device) if fp8 else None,
                    "v_scale": torch.ones(scale_shape, dtype=torch.float32, device=device) if fp8 else None,

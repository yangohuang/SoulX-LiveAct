...
It establishes a reproducible way to reject a misleading one-number claim: temporal stability must consider delayed spikes, total motion, visual action timing and independent lip-sync/identity checks. In the initial identity, anchors lower several image-change metrics, but the stronger option does not pass a multi-objective quality gate because its lip-sync proxy worsens and natural gesture quality is unresolved. The face-reference curve supplies no clear identity advantage. It does **not** establish a general algorithmic improvement, physical consistency, long-form identity preservation, or a LiveAct model repair. The extension below tests whether this diagnostic pattern repeats across two more identities.
...
Both [baseline](artifacts/videos/image2-baseline-30s.mp4) and [anchor 0.45](artifacts/videos/image2-anchor045-30s.mp4) have 722 frames and match the fixed format. A [30-second side-by-side review video](artifacts/videos/image2-baseline-left-anchor045-right-30s.mp4) places baseline on the left and the anchor on the right with the shared source audio. The [motion JSON](artifacts/2026-09-25-image2-motion-pair.json), [face-reference JSON](artifacts/2026-09-25-face-identity-image2.json), [SyncNet record](artifacts/2026-09-25-image2-syncnet.json) and [seven-timepoint contact sheet](artifacts/2026-09-25-image2-long-horizon-contact.png) preserve the comparison.
...
### Image 3, seed 42: second completed extension pair

Both [baseline](artifacts/videos/image3-baseline-30s.mp4) and [anchor 0.45](artifacts/videos/image3-anchor045-30s.mp4) decode to 722 frames at 416×720 and 24 FPS. The [motion JSON](artifacts/2026-09-25-image3-motion-pair.json), [face-reference JSON](artifacts/2026-09-25-face-identity-image3.json), [SyncNet record](artifacts/2026-09-25-image3-syncnet.json), [seven-timepoint contact sheet](artifacts/2026-09-25-image3-long-horizon-contact.png) and [frames 596–603](artifacts/2026-09-25-image3-window596-603.png) preserve the evidence. The guitar and hands move throughout; the contact sheets are sparse and do not certify natural timing or hand anatomy.

| Measure | Baseline | Anchor 0.45 | Interpretation |
|---|---:|---:|---|
| Mean eight-transition peak | 5.704 | 4.583 | Lower image-change peak, −19.6% |
| Mean eight-transition motion sum | 34.968 | 26.040 | Also 25.5% less local image change |
| Windows whose peak is after the seam | 15/22 | 21/22 | More delayed local maxima, a warning against seam-only scores |
| Mean face-reference cosine, 31/31 detected | 0.743 | 0.734 | No identity-proxy gain |
| Last-third reference cosine | 0.722 | 0.683 | Pose/expression confounding; not proof of accumulated drift |
| SyncNet minimum distance, matched 754-frame crops | 8.283 | 8.365 | Higher is unfavorable |
| SyncNet relative confidence, matched 754-frame crops | 5.586 | 5.385 | Lower is unfavorable; best offset remains −1 |

The baseline has its largest eight-transition peak in the window starting at transition 596 (7.653 at offset 2); the anchor's corresponding window peaks at 5.617 at offset 7. The frame sequence shows a different mouth and strumming-hand trajectory, so the lower grayscale peak is not a visually established improvement. This pair used the same image-3 reference, the first 30 seconds of audio 3 and seed 42 in separate runs. The exact input hashes and 16-kHz trim details are in the [input manifest](artifacts/2026-09-25-long-horizon-input-manifest.json). Audio 3 has two channels, whereas audio 2 has one; comparisons are paired *within* identity, and absolute SyncNet scores across identities should not be treated as a controlled audio test.

The image-3 baseline MP4 has a container-reported duration of 30.209 seconds despite a 30.083-second, 722-frame video stream and a 30.000-second audio stream. The anchor MP4 reports 30.084 seconds. The SyncNet pipeline's 25-FPS conversion therefore produced 757 versus 754 face-track frames; the raw-score pair is excluded from the comparison. For a matched 754-frame score, the baseline's last three *converted face-crop* frames were removed with an FFmpeg stream-copy `-t 30.16`; the anchor's 754-frame face crop is unchanged. Both crops retain 481,536 audio samples. The [SyncNet record](artifacts/2026-09-25-image3-syncnet.json) gives source/crop hashes and both raw and matched scores. This endpoint correction does not change the original 722-frame motion or identity evaluations.

### Across the three identities

The [paired trade-off chart](artifacts/2026-09-25-cross-identity-tradeoff.png) keeps boundary peak, total local image change and SyncNet confidence on separate axes; it deliberately avoids a combined quality score.

These are three *paired cases at one seed*, not three independent replications across prompts, seeds or model checkpoints. The fixed 0.45 anchor lowers the mean eight-transition grayscale peak by 34.1%, 25.2% and 19.6%, respectively. The same windows lose 29.3%, 28.6% and 25.5% of their summed image change. SyncNet relative confidence falls in all three cases: 7.079→6.599, 6.668→6.397 and 5.586→5.385, with no change in each pair's best offset. The reference-image face cosine mean also decreases in all three cases (0.734→0.724, 0.714→0.696, 0.743→0.734), but this proxy is sensitive to pose and expression and cannot alone establish identity deterioration. The image-2 hand ghosting and the image-3 increase in delayed peaks show why neither reduced MAE nor unchanged face detection is a sufficient stability criterion. There is no consistent early-to-late motion-peak drift across the three cases: image 1 declines, while images 2 and 3 rise. Thirty seconds cannot resolve hour-scale accumulation.

The outcome is a useful *negative method result*: this anchor reliably changes local image statistics in these cases, yet the current evidence does not justify shipping it as a general long-video fix. The reusable contribution is the evaluation contract, the explicit countermetrics and the documented false-positive modes. A next inference candidate must preserve action timing, hand clarity, mouth synchronization and identity under a measured extra-pass/latency budget; otherwise its lower seam score is rejected.

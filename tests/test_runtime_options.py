...
class RuntimeOptionsTests(unittest.TestCase):
    def test_existing_compile_and_pinning_defaults_are_preserved(self):
        ...
        self.assertFalse(args.pageable_block_memory)

    def test_low_memory_compile_and_pageable_flags_are_explicit(self):
        parser = argparse.ArgumentParser()
        add_low_memory_arguments(parser)

        args = parser.parse_args(["--disable_compile", "--pageable_block_memory"])

        self.assertTrue(args.disable_compile)
        self.assertTrue(args.pageable_block_memory)
